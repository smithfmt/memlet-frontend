...
const WordPair = (props) => {
    const { wordpair, focusInput, learntThis, index } = props;
    ...
    return (
        ...
        <form spellCheck="false" className={`word-pair-container ${deleting}`}>
            ...
            <button id={`wordpairForm-learnt-${props.index}`} className={`edit-learnt-button ${!wordpair.learnt?"learnt":""}`} onClick={(e) => {e.preventDefault();learntThis(wordpair.id, !wordpair.learnt, index)}} /> 
        </form>
